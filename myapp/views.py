...
def dynamic_delete(request,id):
    ...
    chatTitles = ChatTitle.objects.first()
    chatBodys = ChatBody.objects.filter(chatTitle_id=chatTitles.id).order_by('id')
    chatBodyscount = ChatTitle.objects.all().count()
...
def chat_add(request):
    ...
    # report_test_list = {'id':1,'title':'title','commission':14,'price':14}

    # firstValue = int(firstValue)
    # print('inn',type(firstValue))
    # converted_num = int(firstValue)
    
    if 'counter' not in request.session:
        request.session['counter'] = 0

    
    # ChatTitle.objects.all().delete()
    # ChatBody.objects.all().delete()

    

    # print(f"Counter value: {request.session['counter']}")
    ...
    print('chatTitle',request.session['chatTitleID'])
    print('mychatTitleID',mychatTitleID)
...
def chatui(request):
    ...
    return render(request, 'chatui.html',data)
